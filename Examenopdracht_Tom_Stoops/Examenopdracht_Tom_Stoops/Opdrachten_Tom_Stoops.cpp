...
void Opdracht_1() { // Uitwerking van opdracht 1
	std::cout << "\n\n\t==================================" << std::endl;
	std::cout << "\t|| Opdracht 1: Source-Sink paar ||" << std::endl;
	std::cout << "\t==================================" << std::endl;
	Grid opdr1(-2, 2, -1, 1, 200, 100); // maak het grid aan
	...
	Source bron(-1, 0, 1); // maak de nodige objecten aan
	Source put(1, 0, -1);
	opdr1.addBaseFlow(&bron); // voeg de objecten toe aan de vector van fundamentele oplossingen
	opdr1.addBaseFlow(&put);

	std::cout << "-- Data wegschrijven naar bestanden met signature: 'O1_D1' --" << std::endl; // schrijf de data weg
	...
	std::cout << "\t------------------------------------------------------------" << std::endl;
	opdr1.clearBaseFlow(); // we hergebruiken het grid uit het eerste deel, dus maken we eerst de vector van fundamentele oplossingen leeg

	Source put2(1, 0, -0.25); // maak nieuw object
	opdr1.addBaseFlow(&bron); // we herbruiken een bron uit het eerste deel maar voegen een andere put toe
	opdr1.addBaseFlow(&put2);

	std::cout << "-- Data wegschrijven met signature: 'O1_D2' --" << std::endl; // schrijf de data weg
	...
	std::cout << "\t--------------------------------------------------------------" << std::endl;
	opdr1.clearBaseFlow(); // maak de vector van fundamentele oplossingen leeg

	std::cout << "-- We zorgen dat Q*l = 1 => gelijkwaardig aan een doublet met sterkte 1 voor l->0. --" << std::endl;
	std::vector<double> l = { 1, 0.5, 0.25, 0.125 }; // vectoren met waarden l en Q die in de loop gebruikt worden, merk op dat voor all i: l.at(i)*Q.at(i) = 1 = cte
	std::vector<double> Q = { 1, 2, 4, 8 };

	std::cout << "-- Data wegschrijven met signature: 'O1_D3_i' waar 'i' de ide iteratie. --" << std::endl;
	for (int i = 0; i < Q.size(); i++) { // loop over de verschillende onderdelen van de opdracht
		std::cout << "||Iteratie " << i+1 << "||" << std::endl; // geef weer op het scherm welke iteratie we behandelen
		Source doublet_bron((-l.at(i) / 2), 0, Q.at(i)); // maak de objecten aan met de parameters gegeven in de vectoren l en Q
		Source doublet_put((l.at(i) / 2), 0, (-Q.at(i)));
		opdr1.addBaseFlow(&doublet_bron); // voeg de objecten toe aan de vector
		opdr1.addBaseFlow(&doublet_put);

		opdr1.writeStream("O1_D3_" + std::to_string(i+1) + "_stream"); // schrijf de data weg, std::to_string zorgt ervoor dat de iteratie ook weergegeven wordt in de bestandsnaam
		opdr1.writePotential("O1_D3_" + std::to_string(i+1) + "_potential");
		opdr1.writeVelocity("O1_D3_" + std::to_string(i+1) + "_vectors");

		doublet_bron.~Source(); // deconstruct de gemaakte objecten, zo kunnen deze aan het begin van de loop opnieuw gemaakt worden onder dezelfde naam maar met andere parameters
		doublet_put.~Source();
		opdr1.clearBaseFlow(); // maak de vector van fundamentele oplossingen leeg aan het einde van de loop
	}
};

void Opdracht_2() { // Uitwerking van opdracht 2

	std::cout << "\n\t=========================================" << std::endl;
	std::cout << "\t|| Opdracht 2: Source in Uniforme flow ||" << std::endl;
	std::cout << "\t=========================================" << std::endl;
	Grid opdr2(-4, 4, -2, 2, 200, 100); // maak grid aan met gegeven parameters

	Source bron(-1, 0, 6); // maak nodige objecten en voeg deze toe aan de vector van fundamentele oplossingen
	Uniform stroming(0, 1);
	opdr2.addBaseFlow(&bron);
	opdr2.addBaseFlow(&stroming);

	std::cout << "-- Data wegschrijven met signature: 'O2' --" << std::endl; // schrijf de data weg
	opdr2.writeStream("O2_stream");
	opdr2.writePotential("O2_potential");
	opdr2.writeVelocity("O2_vectors");
};

void Opdracht_3() { // Uitwerking van opdracht 3
	std::cout << "\n\n\t===================================================" << std::endl;
	std::cout << "\t|| Opdracht 3: Source-Sink paar in Uniforme flow ||" << std::endl;
	std::cout << "\t===================================================" << std::endl;
	Grid opdr3(-2, 2, -1, 1, 200, 100); // maak het grid aan
	...
	Source bron(-1, 0, 1); // maak de nodige objecten aan en voeg deze toe aan de vector van fundamentele oplossingen
	Source put(1, 0, -1);
	Uniform stroom(0, 1);
	...
	std::cout << "-- Data wegschrijven met signature: 'O3_D1' --" << std::endl; // schrijf data weg
	...
	std::cout << "\t---------------------------------------------------------------------------------" << std::endl;
	opdr3.clearBaseFlow(); // maak de vector van fundamentele oplossingen leeg, opdat het grid herbruikt kan worden

	Source bron2(-1, 0, 3); // maak nodig object aan
	opdr3.addBaseFlow(&bron2); // voeg alle nodige objecten toe en herbruik waar mogelijk
	opdr3.addBaseFlow(&put);
	opdr3.addBaseFlow(&stroom);

	std::cout << "-- Data wegschrijven met signature: 'O3_D2' --" << std::endl; // data wegschrijven
	opdr3.writeStream("O3_D2_stream");
	opdr3.writePotential("O3_D2_potential");
	opdr3.writeVelocity("O3_D2_vectors");
};

void Opdracht_4() { // Uitwerking opdracht 4, structuur volledig analoog aan voorgaande opdrachten
	std::cout << "\n\n\t==========================================" << std::endl;
...
void Opdracht_5() { // Uitwerking opdracht 5, structuur volledig analoog aan voorgaande opdrachten
	std::cout << "\n\n\t==========================" << std::endl;
	...
	std::vector<int> N = { 3, 4, 5, 10 }; // Vector met verschillende waarden voor N waarover we zullen loopen

	std::cout << "-- Data wegschrijven met signature: 'O1_D3_i' waar 'i' de ide iteratie. --" << std::endl;
	for (int i = 0; i < N.size(); i++) { // loop over de verschillende iteraties, analoog aan voorgaande iteratie loop (zie source-sink -> doublet)
...
void Opdracht_6() { // Uitwerking opdracht 6, structuur volledig analoog aan voorgaande opdrachten
	std::cout << "\n\n\t====================================" << std::endl;
	...
	Vortex vort2(0, 0, 10 * M_PI); // 10*pi is voldoende om het criterium te breken
	opdr6.addBaseFlow(&stroom);
	opdr6.addBaseFlow(&bronput);
